...
namespace iedb
{
    ...
    int db::create_statement_process(parse_result& result)
    {
        ...
        return data_manager->create_table(*new_table);
    }
    ...
    int db::query_statement_process(parse_result& result)
    {
        //尝试获取对应表
        ...
        // 构建查询结果
        for (auto root = result.filter;root;root=root->brother)
        {
            // auto new_root = root;
            auto new_root = expr::convert_infix_to_suffix(root);
            while (new_root)
            {
                printf("%s ",new_root->to_string().c_str());
                new_root = new_root->child;
            }
    ...
    db::reader db::sql_execute(const char* sql)
    {
        auto result = parse(sql);
        if (result == nullptr || result->type!= token_type::select)
        {
            fprintf(stderr, "错误的SQL语句:%s\n", sql);
            return reader{status_invalid_sql};
        }
        return reader{query_statement_process(*result)};
    }
