...
namespace iedb {
    ...
    pager::pager(int fd, std::unique_ptr<journal> &j, int64 original_file_size): j(std::move(j)), fd(fd),
        page_count(original_file_size / page_size), status(pager_status::error) {
    }
    ...
    */
    ...
    int pager::WritePages() {
        assert(status == pager_status::write_transaction);
        //尝试对需写入页面进行排序
        static std::vector<uint64_t> sortArray;
        sortArray.resize(writable_pages.size());
        auto writeCount = writable_pages.size();
        for (auto i = 0; i< writeCount; i++) {
            sortArray[i] = (static_cast<uint64_t>(writable_pages[i]->get_page_no()) << 32) | i;
        }
        std::sort(sortArray.begin(), sortArray.end());
        for (auto i = 0; i< writeCount; i++) {
            auto writePageId = sortArray[i] & 0xFFFF;
            auto&page = writable_pages[writePageId];
            auto status = os::write(fd,static_cast<int64>(page->page_no) * page_size, page->get_data(), page_size);
